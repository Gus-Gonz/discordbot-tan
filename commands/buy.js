...
module.exports = {
  run: async (discord, bot, msg) => {
    ...
    if (msg.channel.id != "840947787956879397")
      //ID of the channel wher it is allow to use the command buy
      return msg.channel.send(
        `Please use the ${msg.guild.channels.cache.find(
          (c) => c.id == "840947787956879397"
        )} channel.`
      );
    let user = msg.author;
        ...
        (c) =>
          c.setParent(
            msg.guild.channels.cache.find(
              (channel) => channel.id == "840947790888173626" // ID of category where to put the ticket
            ).id
          ),
        (e) => console.log(e)
